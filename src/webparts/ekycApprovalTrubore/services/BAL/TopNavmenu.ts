...
export interface ITopNavmenuOps {
    getTopNavmenuData(props: IEkycApprovalTruboreProps): Promise<ITopNavmenu[]>;
}

export default function TopNavmenuOps(): ITopNavmenuOps {
    const spCrudOps = SPCRUDOPS();

    const getTopNavmenuData = async (props: IEkycApprovalTruboreProps): Promise<ITopNavmenu[]> => {
        try {
