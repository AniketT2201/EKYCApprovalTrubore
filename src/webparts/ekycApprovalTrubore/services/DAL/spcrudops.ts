...
export interface ISPCRUDOPS {
    getData(listName: string, columnsToRetrieve: string, columnsToExpand: string, filters: string, orderby: { column: string, isAscending: boolean }, props: IEkycApprovalTruboreProps): Promise<any>;
    getRootData(listName: string, columnsToRetrieve: string, columnsToExpand: string, filters: string, orderby: { column: string, isAscending: boolean }, props: IEkycApprovalTruboreProps): Promise<any>;
    getItemData(listName: string, id: number, select: string, expand: string, props: IEkycApprovalTruboreProps): Promise<any>;
    ...
    addAttchmentInList(data: File, listName: string, itemId: number, fileName: string, props: IEkycApprovalTruboreProps): Promise<any>;
    getAttachments(listName: string, itemId: number, props: IEkycApprovalTruboreProps): Promise<any[]>;
    uploadAttachment(listName: string, itemId: number, file: File, props: IEkycApprovalTruboreProps): Promise<any>;
    deleteAttachment(listName: string, itemId: number, fileName: string, props: IEkycApprovalTruboreProps): Promise<any>;
    postHttpData(endpoint: string, data: any, props: IEkycApprovalTruboreProps): Promise<any>;
    
}


class SPCRUDOPSImpl implements ISPCRUDOPS {
    async getData(listName: string, columnsToRetrieve: string, columnsToExpand: string, filters: string, orderby: { column: string, isAscending: boolean }, props: IEkycApprovalTruboreProps): Promise<any> {
    ...
    async getRootData(listName: string, columnsToRetrieve: string, columnsToExpand: string, filters: string, orderby: { column: string, isAscending: boolean }, props: IEkycApprovalTruboreProps): Promise<any> {
        if (!props.currentSPContext || !props.currentSPContext.pageContext) {
            throw new Error('SharePoint context is not available');
        }
    ...
    async getItemData(listName: string, id: number, select: string, expand: string, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
    ...
    async insertData(listName: string, data: any, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(listName).items.add(data);
    }

    async updateData(listName: string, itemId: number, data: any, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(listName).items.getById(itemId).update(data);
    }

    async deleteData(listName: string, itemId: number, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(listName).items.getById(itemId).delete();
    }

    async getListInfo(listName: string, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(listName).get();
    }

    async getListData(listName: string, columnsToRetrieve: string, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        let items = web.lists.getByTitle(listName).items;
        if (columnsToRetrieve) {
            items = items.select(columnsToRetrieve);
    ...
    async batchInsert(listName: string, data: any, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
    ...
    async batchUpdate(listName: string, data: any, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        const batch = web.createBatch();
        data.forEach((item: any) => {
    ...
    async batchDelete(listName: string, data: any, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        const batch = web.createBatch();
        data.forEach((item: any) => {
    ...
    async createFolder(listName: string, folderName: string, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(listName).rootFolder.folders.addUsingPath(folderName);
    }

    async uploadFile(folderServerRelativeUrl: string, file: File, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.getFolderByServerRelativeUrl(folderServerRelativeUrl).files.add(file.name, file, true);
    }

    async deleteFile(fileServerRelativeUrl: string, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.getFileByServerRelativeUrl(fileServerRelativeUrl).delete();
    }

    async currentProfile(props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.currentUser.get();
    }

    async getLoggedInSiteGroups(props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.currentUser.groups.get();
    }

    async getAllSiteGroups(props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
    ...
    async addAttchmentInList(data: File, listName: string, itemId: number, fileName: string, props: IEkycApprovalTruboreProps): Promise<any> {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(listName).items.getById(itemId).attachmentFiles.add(fileName, data);
    }
    async getAttachments(listName: string, itemId: number, props: IEkycApprovalTruboreProps): Promise<any[]> {
        try {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
    ...
    async uploadAttachment(listName: string, itemId: number, file: any, props: IEkycApprovalTruboreProps): Promise<any> {
        try {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        await web.lists
            .getByTitle(listName)
    ...
    async deleteAttachment(listName: string, itemId: number, fileName: any, props: IEkycApprovalTruboreProps): Promise<any> {
        try {
        const web = Web(props.currentSPContext.pageContext.web.absoluteUrl);
        await web.lists
            .getByTitle(listName)
        ...
        // POST
    async postHttpData(endpoint: string, data: any, props: IEkycApprovalTruboreProps): Promise<any> {
